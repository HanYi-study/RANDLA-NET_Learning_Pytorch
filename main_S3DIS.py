import warnings  #用于处理非致命性程序警告的标准库工具.管理程序运行过程中出现的各种警告信息，这些警告指示潜在问题但不会像异常那样终止程序执行。
from helper_tool import ConfigS3DIS as cfg  #configs3dis 是一个专门用于处理 S3DIS 数据集的配置工具包，主要用于三维点云场景分割任务
...
parser.add_argument('--test_area', type=int, default=5,
                    help='Which area to use for test (others use to train), option: 1-6 [default: 6]')
