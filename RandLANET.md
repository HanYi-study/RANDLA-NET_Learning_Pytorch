...
- 膨胀残差块”通常指的是结合了残差结构（Residual Block）和膨胀卷积（Dilated Convolution，又称空洞卷积）的网络结构单元。
- 残差块（Residual Block）
    - 典型于 ResNet，核心思想是“跨层直连”，即将输入加到输出上（shortcut connection）。
    - 形式：output = F(x) + x，其中 F(x) 是若干卷积、归一化、激活等操作。
- 膨胀卷积（Dilated Convolution）
    - 普通卷积核之间插入“空洞”，用一个膨胀率（dilation rate）控制，能在保持参数量不变的情况下扩大感受野。
    - 适用于需要捕获大范围特征但不希望引入过多参数的场景。
- 膨胀残差块（Dilated Residual Block）
    - 结构：用膨胀卷积替代普通卷积，嵌入在残差块内部。
    - 优势：同时获得宽感受野和高效的信息流，适合处理结构复杂或者空间关系丰富的数据。
- 使用方法：在搭建网络结构时，将膨胀残差块堆叠在主干网络的不同层级，在代码实现时只是把卷积层替换为膨胀卷积。
- 作用：
    - 扩大感受野，在不增加参数数量的情况下，让卷积核看到更大范围的数据，有利于捕获全局和长距离的信息。
    - 保证梯度流畅，残差链接让梯度更容易反向传播，避免深层网络训练中的梯度消失问题
    - 增强特征提取能力，可以更好融合局部和全局特征
- 输出数据和输入数据的维度通常一致（保持shape不变），但特征内容被增强了。得到的时特征张量，包含了融合了更大感受野、更加丰富的上下文信息的表示。这些特征将被送到后续的网络层，最终用于分类、分割等任务的决策。
2. 为什么要进行编码器操作：
